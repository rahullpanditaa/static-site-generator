...
def generate_page(from_path, template_path, dest_path):
    ...
    with open(source_file_path, "r") as source, open(template_file_path, "r") as template, open(dest_file_path, "w") as destination:
        ...
        source_node_string = markdown_to_html_node(source_file_contents).to_html() # source md -> html
        source_title = extract_title(source_file_contents)
        template_file_contents = template_file_contents.replace("{{ Title }}", source_title).replace("{{ Content }}", source_node_string)
